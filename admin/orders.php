        ...
        <div class="table-data">
				...
				<div class="left">
					...
					<table id="myTable">
						...
						<tbody>
						<?php 
$sql ="SELECT tblusers.id, tblusers.FullName, tblorders.id as order_id,tblorders.orderdate, tblorders.orderstatus,tblorders.total_amount,tblorders.mop, tblorders.cust_id 
FROM tblusers
...
$('#myTable').DataTable({
    ...
    var currentStatusSelect;  // To hold the current select element

    // Change event for "status" select element
    $(document).on('change', '.status', function() {
        if ($(this).val() == 'Completed') {
            // Get order details from the select data attribute
...
$.ajax({
    ...
    sowIds[sowId]++;
        ...
        if(weight < minweight || weight > maxweight){
            setTimeout(function() {
    swal("Error", "Inputted weight is not in range with the weight class", "error");
  }, 100);
            // alert('');
            hasError = false;
            return false;
...
let date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
    ...
    $('#myModal').on('hidden.bs.modal', function (e) {
        ...
        if (currentStatusSelect.val() == 'Completed' && !currentStatusSelect.is(':disabled')) {
            // Set the status back to 'Pending' if the modal is cancelled
            currentStatusSelect.val('Pending');
        }
    });
